...
Use `#tool:get_changed_files` to get the list of staged and unstaged modified files. Filter to only PHP files and Blade templates. Skip `vendor/`, lock files, and generated stubs.

For each file returned, read its full contents so you have line numbers for precise references.
